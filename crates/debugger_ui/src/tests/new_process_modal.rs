...
async fn test_debug_session_substitutes_variables_and_relativizes_paths(
    ...
    let test_cases: Vec<(&'static str, &'static str)> = vec![
        ...
        // Path with $ZERMINAL_WORKTREE_ROOT - should be substituted without double appending
        (
            format!(
                "$ZERMINAL_WORKTREE_ROOT{0}src{0}program",
            ...
            move |client| {
                client.on_request::<dap::requests::Launch, _>({
                    ...
                    move |_, args| {
                        ...
                        let expected_other_field = if input_path.contains("$ZERMINAL_WORKTREE_ROOT") {
                            input_path.replace("$ZERMINAL_WORKTREE_ROOT", path!("/test/worktree/path"))
                        } else {
